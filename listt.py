...
def get():
    if request.args.get("id") is not None and request.args.get("id").startswith('tt'):
        hxs = lxml.html.document_fromstring(requests.get("http://www.imdb.com/search/title?year=2013,2013&title_type=feature&sort=moviemeter,asc" + request.args.get("id")).content)
        movie = {}
        try:
