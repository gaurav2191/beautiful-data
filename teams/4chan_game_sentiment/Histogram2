...
with open('4chandata.txt', 'r') as f:
    avg = [int(float(line.strip())) for line in f]
print avg
...
with open('metacritic_score_list.txt', 'r') as f:
	meta_score = [int(float(line.split()[2])) for line in f]
	user_score = [float(line.split()[3]) for line in f]
...
rects2 = plt.bar(index + bar_width, user_score, bar_width,
                 alpha=opacity,
                 color='r',
		 yerr=release;
                 error_kw=error_config,
